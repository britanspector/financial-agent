...
from financial_agent.user_data.runtime import build_local_user_tools


def test_environment_composition_and_secret_exclusion(monkeypatch, db_path, tmp_path):
    secret = "synthetic-runtime-test-key"
    records = [{"api_key": secret, "principal_id": "synthetic-runtime", "user_ids": ["syn-user-0001"],
                "scopes": ["read:customer_context"]}]
    ...
    registry = build_local_user_tools(settings)
    result = registry.invoke("get_customer_context", {"user_id": "syn-user-0001"}, context=context)
...
def test_unconfigured_runtime_denies_access(tmp_path):
    registry = build_local_user_tools(Settings(audit_path=tmp_path / "audit.jsonl"))
    result = registry.invoke("get_customer_context", {"user_id": "syn-user-0001"}, context=CallContext())
...
def test_missing_database_is_safe_and_not_created(tmp_path):
    ...
    result = build_local_user_tools(settings).invoke("get_customer_context", {"user_id": "syn-user-0001"}, context=CallContext(api_key=key))
